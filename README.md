...
- 開発リーダーとして開発タスク管理、 CMS/API/DB設計、実装、単体テストを担当。
- チームメンバーへのタスク指示及び技術サポート。
- 要件定義にてクローズドサイトの開発にあたっての要件及び確認事項の整理。
- デザイナー、アプリエンジニアとの仕様認識合わせ及び社内調整。
- イベントを作成するCMSにクローズドサイトのイベントを作成できる機能の設計、開発を担当。
- イベント関連APIでのクローズドサイトを含むものに対してのAPI改修の設計、開発を担当。
- クローズドサイトの設計、開発を担当。
- Postパラメータ認証、自然検索にインデックスされないためのサイトアクセス制御対応。
- ランダム文字列生成アルゴリズムを用いる推測不可のURL生成の実装。
- OSに基づいたストアまたはインストール済みアプリに遷移するフロントエンド処理の実装。
