...
### 大手レコード会社向けチケットアプリのAPI開発/CMS保守/サイト改修【TypeScript, Vue, Laravel, AWS】(2022年〜2023年)
...
【発揮したバリュー】
APIリプレイスの要件定義から案件に参画し、設計、開発、納品、保守まで一貫して担当。Laravelでの型定義を必須としたコーディングや各APIにおけるユニットテスト、結合テストのテストコードを作成し、テストカバレッジをリプレイス開発期間内に90％以上達成し大きな不具合等なくAPIリプレイスが完了、管理者用CMSの保守作業、イベント詳細サイトの改修では、TypeScript、Vueの組み合わせは初めてであったが、Composition APIの初期フェーズでキャッチアップを行いプロジェクトに貢献。
...
【発揮したバリュー】
オンラインのコミュニケーションベースで行うマネジメント業務は初めてであったが、チームメンバーとの信頼関係を構築することに焦点を置き、相談のしやすくタスク等も依頼しやすい関係を築くことができ、プロジェクトを円滑に進めることができた。
...
- 複数テーブルのリレーションを意識したAPI開発。

【発揮したバリュー】
新規開発フェーズから参画したことでプロジェクトの多くのAPI開発に携わりプロジェクトに貢献。
実装と合わせて必ずメソッド単位でユニットテストを作成し、テストカバレッジは90％以上を達成できた。
