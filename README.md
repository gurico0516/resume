...
【プロジェクト概要】
アーティストやアニメのイベント系のチケットアプリのAPIリプレイス開発、チケットアプリ管理用のCMS保守、イベント詳細サイトの改修。

【担当業務】APIのリプレイス開発、アプリのAWS保守運用、運営管理者用CMSの保守、イベント詳細サイトの改修を担当。具体的には下記。
- チケットアプリと顧客情報等を含む外部APIの橋渡しとなる中間サーバーAPIの開発。
- Rest API定義によるAPI設計。
- チームメンバー内でのコートレビュー作業。
- ユーザーから不具合の問い合わせがあった際には、Cloudwatchでログ調査で原因特定し、ソースコード修正、DB不整合による本番DBのデータ修正。
...
- チケットアプリに紐づくイベント詳細サイトの改修を担当。イベント詳細サイトの該当イベント選択時にios、android、pcの判別を行い、pc以外の場合はそれぞれのOSに基づいたストアまたはインストール済みアプリに遷移する処理をTypeScript, Vueで作成。該当イベントのデータをDBよりLaravelでデータを取得し、TypeScript、Vueとの連携、イベントの手数料や支払い方法を表示するために、外部APIをコールする必要があり、その仕組みをLaravelで作成し、Typecript、Vueとの連携を実施。
