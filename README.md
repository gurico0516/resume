...
- Mermaidを利用したER図作成。
- 各APIに対するログ設定をlogging.basicConfigを用いて実装。
- GitHub Actionsを用いた以下のCIパイプラインの構築
  - flake8を用いた静的解析とPEP8規約検査の自動テスト。
  - blackによる自動整形スクリプトの作成。
  - flake8に基づいたreviewdogによる自動コードレビュースクリプトの作成。
  - pip-auditを用いたセキュリティ脆弱性スキャンツールの自動スクリプト。
...
- ログレベル、ログメッセージの選定。
- MongoDBからドキュメントデータの取得とオブジェクトストレージに保存されたテキストファイルのベクトル化。
- バッチサーバーを介してLangChainサーバーにテキストファイルを送信し、ベクトル化を行うプロセスの実装。

【発揮したバリュー】
LLMベースのデータ統合・解析プラットフォームのPoC開発では、効率的なAPIサーバーの設計・実装を行い、迅速で効果的なデータ解析機能を提供しました。また、コンテナ環境の整備により、開発環境のセットアップを簡素化し、チーム全体の生産性を向上させました。さらに、GitHub Actionsを活用したCIパイプラインの導入で、コードの品質とセキュリティを確保。そのほか、クローラーバッチの実装により、大量のデータを効率的に収集し、解析の基盤を構築しました。ベクトル化バッチの開発を通じて、データの詳細な解析を実現。また、OAuth2.0認証の導入により、セキュアなユーザー認証を提供しました。これらの取り組みにより、プロジェクトに貢献。
